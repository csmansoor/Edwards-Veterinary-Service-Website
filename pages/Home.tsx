...
const Home: React.FC = () => {
  ...
  const serviceList = [
    { title: "Pet Wellness & Preventive Care", slug: "pet-wellness-care", desc: "Comprehensive wellness exams to ensure your pets live a long, happy, and healthy life.", img: wellnessExamImg },
    { title: "Vaccinations for Dogs & Cats", slug: "puppy-kitten-care", desc: "Customized immunization plans to protect your furry family members from common diseases.", img: puppyVaccineImg },
    { title: "Pet Dentistry & Dental X-Rays", slug: "pet-dentistry", desc: "Complete oral health services including professional cleaning and digital dental imaging.", img: dentistryImg },
    { title: "In-House Diagnostics & Radiology", slug: "pet-x-ray", desc: "State-of-the-art digital X-rays and diagnostics for fast and accurate medical results.", img: diagnosticsImg },
    { title: "Mobile & At-Home Services", slug: "pet-mobile-services", desc: "Bringing the vet to you. Stress-free veterinary care in the comfort of your own home.", img: mobileServicesImg }
  ];
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (!isServicesHovered && !isServicesTouched && !arrowScrollingRef.current) {
  ...
  return (
    <div className="overflow-x-hidden">
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="flex flex-col lg:flex-row items-center gap-16">
            ...
            <div className="lg:w-1/2">
              <h2 className="text-2xl md:text-5xl font-bold text-[#008000] mb-6 leading-tight">Your Local, Community-Rooted Veterinarian in Tillsonburg, ON</h2>
              <p className="text-gray-600 text-base md:text-lg mb-6 leading-relaxed">For more than three decades, Edwards Veterinary Services has been proud to serve pets across Tillsonburg.</p>
              <Link to="/about" className="inline-block bg-[#008000] text-white font-bold py-3 px-6 md:py-4 md:px-8 rounded-md uppercase tracking-wider hover:bg-[#006400] transition shadow-lg text-sm md:text-base">
                Learn More About Us
              </Link>
            </div>
          </div>
      ...
      <section className="py-16 md:py-24 bg-[#008000]">
        <div className="container mx-auto px-4 max-w-5xl text-center text-white">
          <h2 className="text-2xl md:text-5xl font-black uppercase mb-8 tracking-tight">Comprehensive Veterinary Services <br className="hidden md:block" /> In Tillsonburg, ON</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12 text-left max-w-3xl mx-auto">
            {[
              { title: "24/7 Emergency Care", note: "Existing clients only", slug: "pet-emergency-care" },
              { title: "Urgent Care", note: "Business hours only", slug: "pet-same-day-urgent-care" },
              { title: "Mobile / Home Visits", slug: "pet-mobile-services" },
              { title: "Exotics Pet Care", slug: "exotic-pet-care" },
              { title: "Advanced Diagnostics", slug: "in-house-lab" },
              { title: "Pet Surgery", slug: "pet-soft-tissue-surgery" }
            ].map((item, index) => (
              <Link key={index} to={`/services/${item.slug}`} className="flex items-start group/item transition-transform hover:translate-x-2">
                <svg className="w-5 h-5 md:w-7 md:h-7 mr-3 md:mr-4 flex-shrink-0 mt-1 text-white group-hover/item:text-yellow-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path>
                </svg>
                <div>
                  <span className="text-lg md:text-2xl font-bold block group-hover/item:text-yellow-400 transition-colors">{item.title}</span>
                  {item.note && <span className="text-sm md:text-base text-gray-200 font-medium italic">{item.note}</span>}
                </div>
              </Link>
            ))}
          </div>
      ...
      <section className="py-24 bg-white overflow-hidden relative group">
        <div className="container mx-auto px-4 mb-12 md:mb-16 text-center">
          <h2 className="text-3xl md:text-5xl font-black text-[#008000] uppercase tracking-tight mb-4">Our Services</h2>
        </div>
          ...
          >
            {infiniteServices.map((service, index) => (
              <Link 
                ...
                key={index} 
                className="flex-shrink-0 w-[280px] md:w-[420px] bg-gray-50 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden border border-gray-100 shadow-sm group/card block hover:shadow-lg transition-shadow duration-300"
              >
                ...
                <div className="p-6 md:p-10">
                  <h3 className="text-xl md:text-2xl font-bold text-gray-800 mb-2 md:mb-4 leading-tight group-hover/card:text-[#008000] transition-colors">{service.title}</h3>
      ...
      <section className="py-20 bg-red-600 text-white">
        <div className="container mx-auto px-4 text-center">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-2xl md:text-5xl font-black uppercase mb-4 tracking-tighter">24-Hour Emergency Veterinary Care</h2>
            <p className="text-base md:text-xl mb-10 leading-relaxed">
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12 md:mb-16">
            <h2 className="text-2xl md:text-5xl font-black text-[#008000] uppercase mb-4">What Our Clients Are Saying</h2>
          </div>
          <div
