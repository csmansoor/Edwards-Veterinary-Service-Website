...
const Appointment: React.FC = () => {
  ...
  // Detection which formee button was clicked on the form page
  useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 1. Medications Array to a single string for the email
    ...
    // 2.  hidden inputs to force data into the email
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
          ...
          <div className="bg-[#008000] p-8 text-white relative">
            ...
            <div className="text-center">
              <h1 className="text-xl md:text-3xl font-bold uppercase tracking-tight">{formType}</h1>
              <p className="mt-1 md:mt-2 opacity-90 italic text-xs">Please complete all mandatory fields marked with *</p>
            </div>
          </div>
          ...
          <form ref={form} onSubmit={handleSubmit} className="p-6 md:p-12 space-y-8">
            ...
            {/*  Button bg*/}
            <button disabled={status === 'sending'} type="submit" className="w-full bg-[#008000] text-white py-4 md:py-5 rounded-2xl font-black text-lg md:text-xl hover:bg-black transition-all shadow-xl disabled:opacity-50">
