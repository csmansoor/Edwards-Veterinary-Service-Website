        ...
        <div className="space-y-6 md:space-y-8">
          {openings.map((job, i) => (
            <div key={i} className="bg-gray-50 p-6 md:p-10 rounded-[2rem] border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-sm hover:shadow-md transition-shadow">
              ...
              <a
                 href={`https://mail.google.com/mail/?view=cm&to=adm.edwardsvet@gmail.com&su=Job%20Application%20-%20${encodeURIComponent(job.title)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="w-full md:w-auto bg-[#008000] text-white px-8 py-4 rounded-full font-black uppercase tracking-widest hover:bg-[#006400] transition-all shadow-lg active:scale-95 text-sm md:text-base text-center inline-block"
              >
                Apply Now
              </a>
            </div>
